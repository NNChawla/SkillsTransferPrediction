...
with open("./borutaFringe.json", "r") as f:
    boruta_features = json.load(f)

subset_features_A = tdf_A[[f"{i}_A" for i in boruta_features]]
subset_features_B = tdf_B[[f"{i}_B" for i in boruta_features]]
...
fs = JointBorutaShap(model=xgb, cv=inner_cv, importance_measure='perm', classification=True, n_trials=50)
_ = fs.fit(subset_features_A, score_A, subset_features_B, score_B, sample_weight_A, sample_weight_B, use_sampling=True)

with open("./boruta_results/fs_boruta_perm_no_sampling.pkl", "wb") as f:
    pickle.dump(fs, f)