...
def vote_step_predictions(step_labels, margins=None, probs=None):
    """
    Aggregate step-level predictions into one task label.

    Parameters
    ----------
    step_labels : ndarray of shape (n_steps,)
        Hard labels from the classifier.
    margins : ndarray of shape (n_steps,), optional
        Signed distances (decision_function output).
    probs : ndarray of shape (n_steps, n_classes), optional
        Probabilities from predict_proba.  probs[:, j] must correspond
        to class j encoded in clf.classes_[j].

    Returns
    -------
    int  # the winning class label (same dtype as step_labels)
    """
    classes = np.unique(step_labels)

    if VOTE_STRATEGY == "majority":
        counts = {c: np.sum(step_labels == c) for c in classes}
        return max(counts, key=counts.get)

    if VOTE_STRATEGY == "prob_sum":
        if probs is None:
            raise ValueError("Need probs for probability voting")
        conf = {c: np.sum(probs[step_labels == c, j])
                for j, c in enumerate(classes)}
        return max(conf, key=conf.get)

    if VOTE_STRATEGY == "tanh_margin":
        if margins is None:
            raise ValueError("Need margins for tanh-margin voting")
        scale = TANH_ALPHA
        if scale is None:
            scale = 1.0 / (np.median(np.abs(margins)) + 1e-9)
        weights = np.tanh(scale * margins)
        conf = {c: np.sum(np.abs(weights[step_labels == c]))
                for c in classes}
        return max(conf, key=conf.get)

    raise ValueError(f"Unknown strategy {VOTE_STRATEGY!r}")
            ...
            for inner_step, (inner_train_idx, inner_test_idx) in tqdm(enumerate(inner_splits), desc="Hypopt CV Splits"):
                ...
                for pid in uniq_pids:
                    ...
                    if mask_A.any():
                        ...
                        if VOTE_STRATEGY == "prob_sum":
                            probs = clf.predict_proba(Za)
                        else:
                            probs = None
                        
                        pred_A = vote_step_predictions(step_lbls,
                               margins=margins,
                               probs=probs)
                    ...
                    if mask_B.any():
                        ...
                        if VOTE_STRATEGY == "prob_sum":
                            probs = clf.predict_proba(Zb)
                        else:
                            probs = None
                        pred_B = vote_step_predictions(step_lbls,
                               margins=margins,
                               probs=probs)
        ...
        for train_idx, test_idx in outer_splits:
            ...
            for pid in np.unique(PID_test):
                ...
                if mask_A.any():
                    ...
                    pred_A   = vote_step_predictions(step_lbls, margins=margins, probs=probs)
                    preds_A.append(pred_A)
                    true_A.append(y_test[mask_A][0])  # same label for all steps of pid
                ...
                if mask_B.any():
                    ...
                    pred_B   = vote_step_predictions(step_lbls, margins=margins, probs=probs)
                    preds_B.append(pred_B)
                    true_B.append(y_test[mask_B][0])
